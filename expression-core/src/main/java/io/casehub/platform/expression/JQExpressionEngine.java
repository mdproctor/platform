...
public class JQExpressionEngine implements ExpressionEngine {
    ...
    public <C, R> CompiledExpression<C, R> compile(String expression, Class<C> contextType, Class<R> resultType, Map<String, Object> variables) {
        ...
        return (CompiledExpression<C, R>) expressionCache.computeIfAbsent(key, k -> {
            ...
            if (resultType == Boolean.class) { jqExpr = new BooleanJQExpression(query, rootScope); }
            else if (resultType == List.class) { jqExpr = new ListJQExpression(query, rootScope); }
            else { jqExpr = new ScalarJQExpression<>(query, rootScope, resultType, MAPPER); }
            if (contextType == JsonNode.class) { return jqExpr; }
    ...
    private JsonQuery compileQuery(String expression) {
        return queryCache.computeIfAbsent(expression, expr -> {
            ...
            catch (Exception e) { throw new ExpressionCompilationException("Failed to compile JQ expression: " + expr, e); }
        });
    }

    private record BooleanJQExpression(JsonQuery query, Scope rootScope) implements CompiledExpression<JsonNode, Boolean> {
        @Override public String type() {return "jq";}
        @Override public Boolean eval(JsonNode context) {
            try {
                Scope childScope = Scope.newChildScope(rootScope);
                List<JsonNode> out = new ArrayList<>();
                query.apply(childScope, context, out::add);
                for (JsonNode node : out) { if (node.isBoolean() && node.asBoolean()) return true; }
                return false;
            } catch (Exception e) { throw new ExpressionEvaluationException("JQ evaluation failed", e); }
        }
    }

    private record ListJQExpression(JsonQuery query, Scope rootScope) implements CompiledExpression<JsonNode, List<JsonNode>> {
        @Override public String type() {return "jq";}
        @Override public List<JsonNode> eval(JsonNode context) {
            try {
                Scope childScope = Scope.newChildScope(rootScope);
                List<JsonNode> out = new ArrayList<>();
                query.apply(childScope, context, out::add);
    ...
    private record ScalarJQExpression<R>(JsonQuery query, Scope rootScope, Class<R> resultType, ObjectMapper mapper) implements CompiledExpression<JsonNode, R> {
        @Override public String type() {return "jq";}
        @Override public R eval(JsonNode context) {
            try {
                ...
                if (resultType == String.class) return resultType.cast(first.asText());
                return mapper.convertValue(first, resultType);
            } catch (Exception e) { throw new ExpressionEvaluationException("JQ evaluation failed", e); }
        }
    }

    private record MapAdaptedJQExpression<R>(CompiledExpression<JsonNode, R> delegate, ObjectMapper mapper) implements CompiledExpression<Map<String, Object>, R> {
        @Override public String type() {return "jq";}
        @Override public R eval(Map<String, Object> context) { return delegate.eval(mapper.valueToTree(context)); }
    }
